...
public class Board extends View {
    ...
    private int cellSize = getWidth()/5;
    ...
    private void GameBoard(Canvas canvas){
    ...
    private void Markers(Canvas canvas){
        for (int r = 0; r < 5; r++){
            for(int c = 0; c < 5; c++){
    ...
    private void horizontalLine(Canvas canvas, int row, int col){
        canvas.drawLine(col, row * cellSize + (float) cellSize / 2,
                cellSize * 5, row * cellSize + (float) cellSize / 2, paint);
    }
    ...
    private void winningLine(Canvas canvas){
        ...
        switch (game.getWinType()[2]){
            ...
            case 2:
                ...
                break;
            case 3:
                diagonalLineNeg(canvas);
                break;
            case 4:
                diagonalLinePos(canvas);
                break;
        }
    }
